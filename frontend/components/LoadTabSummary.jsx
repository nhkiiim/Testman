        ...
        <table class="table w-[1196px] justify-center mx-auto ">
          ...
          <tbody>
            {data?.map((data, index) => (
              ...
              >
                <th>{index + 1}</th>
                <td>{data.resultSummary.numSamples} 개</td>
                <td>{data.resultSummary.avgElapsed} ms</td>
                <td>{data.resultSummary.min} ms</td>
                <td>{data.resultSummary.max} ms</td>
                <td>{data.resultSummary.throughput}</td>
                <td>{data.resultSummary.errorCount} 개</td>
                <td>{data.resultSummary.errorRate}</td>
                <td>{data.resultSummary.receivedPerSec} Bytes</td>
                <td>{data.resultSummary.sentPerSec} Bytes</td>
              </tr>
            ))}
          </tbody>
        </table>
              ...
              <div className="flex w-[1280px] h-[500px] ">
                <div className="border-0 w-full mx-auto rounded-lg shadow-lg overflow-auto flex flex-col  bg-white outline-none focus:outline-none">
                  ...
                  <table class="table w-[1250px] justify-center mx-auto mb-[200px] ">
                    ...
                    <tbody>
                      {data[tabIdx].resultRawList?.map((dt, index) => (
                        <tr className="hover" key={index}>
                          <th>{index + 1}</th>
                          <td>{dt.success ? "성공" : "실패"}</td>
                          <td>{dt.elapsed} </td>
                          <td>{dt.threadName}</td>
                          <td>{dt.grpThreads} </td>
                          <td>{dt.latency}</td>
